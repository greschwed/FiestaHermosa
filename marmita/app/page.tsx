...
export default function Dashboard() {
  ...
  const custoMedio = RECEITAS.reduce((s, r) => s + r.custoPorcao, 0) / RECEITAS.length;
      ...
      <div className="scroll">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
          <div className="card" style={{ padding: 14 }}>
            ...
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, marginTop: 8 }}>{totalReceitas}</div>
            <div style={{ fontSize: 12, color: 'var(--ink-3)' }}>Receitas ativas</div>
          </div>
          <div className="card" style={{ padding: 14 }}>
            <Icon name="package" size={18} color="var(--terracotta)" />
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, marginTop: 8 }}>{totalInsumos}</div>
            <div style={{ fontSize: 12, color: 'var(--ink-3)' }}>Insumos</div>
          </div>
          <div className="card" style={{ padding: 14 }}>
            <Icon name="trending" size={18} color="var(--good)" />
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, marginTop: 8 }}>R$ {fmtNum(custoMedio)}</div>
          ...
          <div className="card" style={{
            ...
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, marginTop: 8 }}>{estoqueBaixo}</div>
        ...
        <div className="row" style={{ marginBottom: 10 }}>
          ...
          <Link href="/receitas" style={{ background: 'none', border: 'none', color: 'var(--terracotta)', fontSize: 13, padding: 0, textDecoration: 'none' }}>Ver todas</Link>
        </div>
          ...
          {RECEITAS.slice(0, 4).map((r, i) => (
            <Link key={r.id} href={`/receitas/${r.id}`} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderBottom: i < 3 ? '1px solid var(--line)' : 'none', textDecoration: 'none', color: 'inherit' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div className="serif" style={{ fontSize: 17, color: 'var(--terracotta)' }}>{fmtBRL(r.precoSugerido)}</div>
              </div>
            </Link>
          ))}
        </div>
